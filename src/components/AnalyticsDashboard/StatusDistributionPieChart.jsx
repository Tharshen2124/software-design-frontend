...
import { GovtProjectsStatusColorMap, ProjectsStatusColorMap, statusColorMap } from "./AnalyticsMapping";
...
export function ProjectsStatusDistributionChart() {
  const [chartData, setChartData] = useState([]);
  ...
  useEffect(() => {
    ...
    fetch(`${backendURL}/dashboard?user_id=${user.id}`, {
      ...
      .then((data) => {
        ...
        const formattedData = Object.entries(breakdown).map(([key, value]) => {
            const cleanKey = key.replace("_percentage", "");
            const meta = ProjectsStatusColorMap[cleanKey];
  ...
  return (
    <div>
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <h2 className="text-xl font-bold text-gray-900 mb-6">
          Assigned Projects Status Distribution
        </h2>
...
export function GovtProjectsStatusDistributionChart() {
  const [chartData, setChartData] = useState([]);
  ...
  useEffect(() => {
    ...
    fetch(`${backendURL}/dashboard?user_id=${user.id}`, {
      ...
      .then((data) => {
        ...
        const formattedData = Object.entries(breakdown).map(([key, value]) => {
            const cleanKey = key.replace("_percentage", "");
            const meta = GovtProjectsStatusColorMap[cleanKey];
